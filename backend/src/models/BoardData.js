class BoardData {
    ...
    placePiece(x) {
        if (
            this.inBounds(x, this.grid) &&
            this.grid[x].length < BoardData.MAX_STACK_HEIGHT
        ) {
    ...
    getNextBlankSpot(column) {
        if (this.inBounds(column, this.grid)) {
            ...
            return nextBlankSpot < BoardData.MAX_STACK_HEIGHT
                ? nextBlankSpot
                : null;
        }
    }
    ...
    checkWin(startX, startY) {
        ...
        const checkDirection = (dx, dy) => {
            ...
            while (
                this.inBounds(x + dx, this.grid) &&
                this.inBounds(y + dy, this.grid[x + dx]) &&
